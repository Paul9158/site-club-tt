...
export default function PagesAdminPage() {
  ...
  async function moveOrder(index: number, direction: -1 | 1) {
    ...
    await Promise.all([
      supabase.from("pages").update({ nav_order: b.nav_order }).eq("id", a.id),
      supabase.from("pages").update({ nav_order: a.nav_order }).eq("id", b.id),
    ]);
    load();
  }
              ...
              <div className="flex items-center gap-3">
                <div className="flex flex-col gap-1">
                  <button
                    ...
                    className="rounded border border-slate-200 px-1.5 text-xs text-slate-500 hover:bg-slate-100 disabled:opacity-30"
                  >
                    ↑
                  </button>
                  <button
